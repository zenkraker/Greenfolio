...
To update the database use this command from the Web project folder (replace `Greenfolio.API` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../Greenfolio.API.Infrastructure/Greenfolio.API.Infrastructure.csproj -s Greenfolio.API.Web.csproj
```
